...
<head>
    ...
    <script type="text/javascript">
	  ...
	  function set_cookie(name, value, expirehours, domain) {
	    ...
	    today.setTime(today.getTime() + (60*60*1000*24*expirehours));
		...
		function click_check() {
		    var today = new Date();
		    var today_str = today.getFullYear() + '' + (today.getMonth() + 1) + '' + today.getDate(); // 오늘 날짜
		    var click_time = get_cookie('click'); 
		    
		    var id = '<%=(String) session.getAttribute("mem_id")%>';
			
			if( id == "null") {
				alert ("로그인 후 다시 시도하세요.");
			} else if(click_time == '' || click_time < today_str){ // 클릭하지 않았거나 클릭했어도 오늘보다 작으면
		        click(today_str); // 클릭 시킴
		        location.href='${path}/waStartClick';
		    } else { // 또 출근 버튼을 클릭 했다면
		        alert("출근은 하루에 한 번만 가능합니다."); 
		    }
		}
		
		function click(value){
		    set_cookie('click', value, 1); //하룻동안 날짜를 기록해둠
		}
									...
									<div class="mb-3">
										...
										<div class="tab-content bg-lighter" id="pills-tabContent-1">
										 ...
										 <form action="waNightClick?${_csrf.parameterName}=${_csrf.token}" method="post">
											  <div align="right">
			                                       <button type="button" onclick="click_check()" class="btn btn-outline-info">출근</button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
